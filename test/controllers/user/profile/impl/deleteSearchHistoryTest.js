...
describe('Test User Delete Search History', function () {
  this.timeout(120000)
  before((done) => {
  ...
  it('Test Redirect Because User is not logged in', (done) => {
    request(app).get('/deleteSearchHistory').then((res) => {
      expect(res.statusCode).to.equal(302)
      done()
    }).catch((err) => done(err))
  })

  it('Test Successful Search History Deletion', (done) => {
    deleteSearchHistoryController.testMode()
